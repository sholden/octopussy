...
class DataLoader
  ...
  def load!
    ...
    buyers_by_id = CSV.new(csv_io('buyer'), headers: buyer_headers, converters: [:all, bool_converter]).index_by{|r| r[:id]}

    vehicle_headers = [:id, :user_id, :year, :make, :model, :trim, :interior_color, :exterior_color, :sticker_price]
    vehicles_by_buyer = CSV.new(csv_io('vehicle'), headers: vehicle_headers, converters: [:all, bool_converter]).group_by{|r| r[:user_id]}

    price_headers = [:vehicle_id, :price]
    prices_by_vehicle = CSV.new(csv_io('price'), headers: price_headers, converters: [:all, bool_converter]).group_by{|r| r[:vehicle_id]}

    option_headers = [:vehicle_id, :description, :opt_code, :is_quick_package, :is_option_package, :is_dio_option, :msrp, :invoice, :opt_kind]
    options_by_vehicle = CSV.new(csv_io('option'), headers: option_headers, converters: [:integer, bool_converter]).group_by{|r| r[:vehicle_id]}
