...
class DataLoader
  ...
  def load!
    ...
    options_by_vehicle = CSV.open(data_path + '/option.csv', headers: option_headers, converters: [:all, bool_converter]).group_by{|r| r[:vehicle_id]}
    ...
    buyers_by_id.each do |buyer_id, buyer_attributes|
      ...
      Sharting.using_key(buyer_attributes[:email]) do
        user = User.create!(buyer_attributes.except(:id))
        ...
        Array(vehicles_by_buyer[buyer_id]).each do |vehicle_attributes|
          ...
          Array(prices_by_vehicle[vehicle_id]).each do |price_attributes|
            ...
            vehicle.prices.create!(price_attributes.except(:vehicle_id))
          end
